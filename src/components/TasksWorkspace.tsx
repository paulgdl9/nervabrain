...
type ColKey = "name" | "status" | "area" | "priority";
...
const STATUSES = ["todo", "doing", "waiting", "done", "abandoned", "archived"];

// Movable columns (Name stays fixed first). Status defaults to the far right.
const DEFAULT_COLS: ColKey[] = ["area", "priority", "status"];
...
const NAME_COL_STORAGE_KEY = "task-name-width";
...
const STATUS_HUE: Record<string, string> = { todo: BLUE, doing: ORANGE, waiting: PURPLE, done: GREEN, completed: GREEN, abandoned: RED, cancelled: RED, canceled: RED, archived: GRAY };
...
function toEditable(note: VaultNote, index: number): EditableTask {
  ...
  return {
    ...
    execKind: sf(note.data.exec_kind).toLowerCase(),
...
export function TasksWorkspace({ tasks, initialSort, initialAppearance }: { tasks: VaultNote[]; initialSort?: "priority"; initialAppearance: WorkspaceAppearancePreference | null }) {
  const { t, valueLabel } = useLanguage();
  const [items, setItems] = useState<EditableTask[]>(() => tasks.map(toEditable));
  ...
  const [sortCol, setSortCol] = useState<ColKey | null>(initialSort ?? null);
  const [sortDir, setSortDir] = useState<SortDir>("asc");
  const [query, setQuery] = useState("");
  const [searchOpen, setSearchOpen] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  const [openPanel, setOpenPanel] = useState<"filter" | null>(null);
  const [editing, setEditing] = useState<{ id: string; field: keyof EditableTask } | null>(null);
  ...
  function toggleEditor(event: React.MouseEvent<HTMLButtonElement>, id: string, field: keyof EditableTask) {
  ...
  useEffect(() => {
    try {
      const c = localStorage.getItem("task-col-order");
  ...
  function persistCols(next: ColKey[]) {
    setColOrder(next);
    try { localStorage.setItem("task-col-order", JSON.stringify(next)); } catch {}
  }
  ...
  const serverMtimes = useRef(new Map(tasks.map((note) => [note.id, note.mtime])));
  const saveRef = useRef<(o: EditableTask, v?: number) => Promise<void>>(async () => undefined);
  ...
  const saveTask = useCallback(async (o: EditableTask, version = saveVersions.current.get(o.id) || 0) => {
  ...
  useEffect(() => { saveRef.current = saveTask; });

  const commit = useCallback((id: string, patch: Partial<EditableTask>, immediate = false) => {
    const version = (saveVersions.current.get(id) || 0) + 1;
    saveVersions.current.set(id, version);
    let next: EditableTask | undefined;
  ...
  const historyCount = useMemo(() => items.filter((item) => isWorkspaceHistory("task", item.status)).length, [items]);
  const includeHistory = showHistory || [...filterStatuses].some((status) => isWorkspaceHistory("task", status));

  const visible = useMemo(() => {
    let list = filterWorkspaceHistory("task", items, includeHistory);
  ...
  const boardItems = useMemo(() => {
    let list = filterWorkspaceHistory("task", items, includeHistory).sort((a, b) => a.order - b.order);
    if (filterAreas.size) list = list.filter((it) => filterAreas.has(it.area));
    if (filterPris.size) list = list.filter((it) => filterPris.has(it.priority || "none"));
    if (filterStatuses.size) list = list.filter((it) => filterStatuses.has(it.status));
    const q = query.trim().toLowerCase();
    if (q) list = list.filter((it) => (it.title + " " + it.description).toLowerCase().includes(q));
    return list;
  }, [items, includeHistory, filterAreas, filterPris, filterStatuses, query]);
  ...
  async function createTask() {
    const res = await fetch("/api/notes", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "create-task", title: t("workspace.newTask"), priority: "medium" }),
    });
    if (!res.ok) return;
    const body = (await res.json()) as { note?: VaultNote };
    if (!body.note) return;
    const minOrder = items.reduce((m, it) => Math.min(m, it.order), 0);
    const fresh = { ...toEditable(body.note, 0), order: minOrder - 100, status: "todo", priority: "medium" };
  ...
  function cellFor(it: EditableTask, col: ColKey) {
    ...
    <div className="obj-workspace">
      <div className="obj-toolbar">
        ...
        <div className="obj-toolbar-right">
          ...
          <button type="button" className="obj-new-btn" onClick={createTask}>
            <Plus size={14} />
            {t("workspace.new")}
          </button>
        </div>
      ...
      {view === "table" ? (
        <div className="obj-table-shell">
          <div className="obj-grid obj-grid-head" style={{ gridTemplateColumns: gridTemplate }}>
            ...
            {colOrder.map((col) => (
              ...
              >
                ...
                <SortHeader label={t(col === "status" ? "workspace.status" : col === "area" ? "workspace.area" : "workspace.priority")} col={col} sortCol={sortCol} sortDir={sortDir} onClick={onSortClick} />
              </div>
            ))}
            <span />
          </div>
              ...
              >
                <div className="obj-grid obj-row" style={{ gridTemplateColumns: gridTemplate }}>
                  ...
                  <div className="obj-name-cell">
                    ...
                    {it.execKind === "prepare" && it.status !== "done" && (
                      <span className="obj-tag obj-tag-prepared" title={t("workspace.preparedHint")}>
                        {t("workspace.prepared")}
                      </span>
                    )}
                  </div>
                  ...
                  {colOrder.map((col) => (
                    <div
                      ...
                      data-label={t(col === "status" ? "workspace.status" : col === "area" ? "workspace.area" : "workspace.priority")}
                    >
                      {cellFor(it, col)}
                    </div>
                  ))}

                  <button type="button" className="obj-row-del" onClick={() => setConfirmDelete(it.id)} title={t("workspace.deleteTask")}>
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
          ...
          {!visible.length && <div className="obj-empty">{t("workspace.noTasks")}</div>}
        </div>
      ) : (
        <BoardView items={boardItems} onPriority={(id, p) => commit(id, { priority: p })} onStatus={(id, s) => commit(id, { status: s })} areaColor={resolveArea} includeHistory={includeHistory} />
      )}
          ...
          <div className="modal-backdrop" role="presentation" onMouseDown={() => setConfirmDelete(null)}>
            <div className="modal-dialog" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}>
              ...
              <div className="modal-actions">
                ...
                <button type="button" className="button danger" onClick={() => deleteTask(confirmDelete)}>{t("workspace.delete")}</button>
              </div>
            </div>
          </div>
        );
...
function BoardView({ items, onPriority, onStatus, areaColor, includeHistory }: {
  ...
  const cols = group === "status"
    ? [...new Set([...STATUSES, ...items.map((item) => item.status)])]
        .filter((status) => includeHistory || !isWorkspaceHistory("task", status))
  ...
  function valueOf(it: EditableTask) {
    return group === "status" ? (it.status || "todo") : (it.priority || "none");
  }
  ...
  return (
    <>
      ...
      <div className="obj-board">
        {cols.map((col) => {
          const list = source.filter((it) => valueOf(it) === col.key);
